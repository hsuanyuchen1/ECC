...
int main(void)
{
    ...
    float SNR;
    long SEED;        
    int ps[64][128]={0};                                /*ps[previous state][time t-1]=information*/
    int cs[64][128];                                    /*cs[current state][time t]=information*/
...
/******************************************decoding***************************************/    
   ...
   {   
       ...
       for(i=0;i<64;i++)
       {
           ...
           out0=(temp0_out1-*(eno1+t))*(temp0_out1-*(eno1+t))+(temp0_out2-*(eno2+t))*(temp0_out2-*(eno2+t));
           out1=(temp1_out1-*(eno1+t))*(temp1_out1-*(eno1+t))+(temp1_out2-*(eno2+t))*(temp1_out2-*(eno2+t));
           if((pmetric[(i>>1)]+out0)<=(pmetric[((i>>1)+32)]+out1))
             {
                 cs[i][127]=csb[0];                                            
                 metric[i]=pmetric[(i>>1)]+out0;
                 for(j=0;j<127;j++)
